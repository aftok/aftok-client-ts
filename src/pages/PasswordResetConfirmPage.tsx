...
export function PasswordResetConfirmPage({
  ...
  const [state, setState] = useState<ResetState>({ type: "validating" });

  useEffect(() => {
    if (!token) {
      setState({ type: "invalidToken" });
      return;
    }
    let cancelled = false;
    async function validate() {
      const result = await caps.validateToken(token!);
      if (cancelled) return;
      if (result.type === "valid") {
        setState({ type: "form", password: "", confirmPassword: "", error: null });
      } else {
        setState({ type: "invalidToken" });
      }
    }
